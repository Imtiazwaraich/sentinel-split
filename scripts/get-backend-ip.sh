...
POD_IP=$(kubectl --context eks-imtiaz-backend get pods -l app=backend='{.items[0].status.podIP}') -o jsonpath
echo "Backend Pod IP: ${POD_IP}"

# Option 2: Service ClusterIP (internal to backend cluster)
SERVICE_IP=$(kubectl --context eks-imtiaz-backend get svc backend-service -o jsonpath='{.spec.clusterIP}')
