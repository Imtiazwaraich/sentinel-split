...
aws eks update-kubeconfig --name eks-imtiaz-gateway --region ${AWS_REGION} --alias eks-imtiaz-gateway >/dev/null 2>&1
aws eks update-kubeconfig --name eks-imtiaz-backend --region ${AWS_REGION} --alias eks-imtiaz-backend >/dev/null 2>&1

# Get backend pod IP
echo "1. Checking backend service..."
BACKEND_IP=$(kubectl --context eks-imtiaz-backend get pods -l app=backend -o jsonpath='{.items[0].status.podIP}')
...
kubectl --context eks-imtiaz-backend exec -it deployment/backend-service -- curl -s http://localhost:8080/health
echo ""

# Get gateway LoadBalancer URL
echo "3. Checking gateway LoadBalancer..."
GATEWAY_URL=$(kubectl --context eks-imtiaz-gateway get svc gateway-proxy -o jsonpath='{.status.loadBalancer.ingress[0].hostname}')
